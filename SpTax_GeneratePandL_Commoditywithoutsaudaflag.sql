...
(C.ExpiredFlag='Y') and                      
(C.EndDate<= @ToDate or C.EndDate> @ToDate) -- Akshara
...
Select Transactionno,Product,Location,EndDate,Clientid,Instrument,Contract,Symbol,Units,qty,'S',0,                      
...
---findout closed contracts----------------------Added by Govind on 29.09.2022    
 ...
 --delete from #CommoditySauda where (Enddate <  @FromDate or Enddate  > @ToDate) or (Enddate >= getdate())      --Akshara
  delete from #CommoditySauda where (Enddate <  @FromDate) or (Enddate >= getdate())
...
from #CommoditySauda                       
where buysell='B'  and SaudaType in ('N','C')  -- Added to include the Expiry contracts  - Added on 12-01-2010                      
...
from #CommoditySauda                       
where                        
buysell='S'   and SaudaType in ('N','C')  -- Added to include the Expiry contracts  - Added on 12-01-2010                      
group by INSTRUMENT,SYMBOL,CONTRACT,isnull(strikeprice,0),Isnull(OptionType,'') ,Product                   
